...
设置里的 **插件市场** 标签页不在 npm CLI 里。在能放下工作区的机器上跑完 `pnpm run build:lib` 后，把四个已构建包目录（`dsh-host-plugin-marketplace`、`dsh-client-ui-settings-plugin-marketplace`、`dsh-api-remotes`、`dsh-client-connection`）拷到虚拟机，再执行 `sudo bash deploy/marketplace/install.sh <该目录>`。再次运行 `install.sh` 会重装 npm 包并丢掉这次叠加；需要再跑一遍市场脚本。细节见[市场包叠加](../.agents/notes/implemented/process/2026-08-14-overlay-marketplace-packages.md)。
