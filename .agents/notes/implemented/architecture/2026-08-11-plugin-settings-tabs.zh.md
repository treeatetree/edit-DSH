...
功能所有权保持明确：`ui-settings-plugins` 拥有“插件”页面与可编辑卡片，`ui-settings-plugin-inventory` 拥有只读清单视图，各条 Host／RPC 路径留在各自的拥有包中。新的“插件”视图只需注册一个 `settings.plugins.tab` 贡献即可加入。插件市场位于侧栏一级入口和对话栏封面上；该决策记录在 [插件市场一级入口](../feature/2026-08-14-plugin-marketplace-primary-entry.md)。
