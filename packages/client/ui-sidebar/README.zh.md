...
侧边栏外壳插件：负责字标、New Session 操作、布局持有的折叠控件、compact 底栏加会话抽屉、布局切换控件、可感知滚动的区域 seat，以及固定在底部的 Settings seat。[ui-workspace](../ui-workspace/README.md) 持有渲染到 `sidebar.workspaces` 的 Workspace 与 Session 浏览器；本包既不派生其中的行，也不持有其视图偏好。折叠到布局拥有的 56px 轨道仍属于本地呈现行为。当布局 owner share 将 `presentation` 设为 `drawer` 时，外壳绘制底栏（会话、新建会话、`sidebar.footer.action`、设置、布局切换）以及覆盖式会话列表，Escape、遮罩和 Session 变化都会关闭该列表。约定：[slot 系统标准](../../../.agents/notes/implemented/architecture/2026-07-22-slot-type-chain-implementation.md)。
...
`SidebarRootComponentProps` 组合布局 owner share、全局 `useSessions` 和 `useWorkspaces` 钩子、已声明的 `sidebar.workspaces` 与 `sidebar.settings` 子 slot，以及注入的 `startSession`、侧边栏切换与 `setShellPreference` 回调。这里没有插件 store。
...
页脚承载 `sidebar.settings` 与布局切换：侧边栏只渲染固定在底部的布局 slot，并共享其栏状态（`wide`）；ui-settings 在此注册触发行和设置面板。布局切换通过 owner share 的 `nextPreference` 调用 `ctx.layout.setShellPreference`。
