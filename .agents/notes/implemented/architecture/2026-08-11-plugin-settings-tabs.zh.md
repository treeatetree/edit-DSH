...
分区拥有方贡献 `configurable` 标签页，由它声明既有的嵌套 `settings.plugin.item` 列表。配置卡片原有的命名空间绑定、草稿状态、校验与写入均保持不变。`@deepseek-ai/dsh-client-ui-settings-plugin-inventory` 向 `settings.plugins.tab` 贡献 `all` 标签页；它的 Host Loader 观察器、生成的 Remote 命名空间、DTO 与搜索语义保持不变。已停用的清单条目会在摘要和详情中省略重复的“未挂载”运行状态，已启用条目仍显示其 Cordis 阶段。`@deepseek-ai/dsh-client-ui-settings-plugin-marketplace` 贡献 `marketplace` 标签页（`order: 5`），用于浏览官方 GitHub 包和 `dsh-plugin` topic 仓库，并通过 `pluginMarketplace` Remote 修改 profile。
...
Settings 只有一行“插件”导航，排在“Agent 预设”之前，包含**插件配置**、**插件市场**与**插件列表**三个标签页。“Agent 预设”仍是独立分区，因为它编辑每个会话的 agent 组装，而非实时 Host Loader 树。

功能所有权保持明确：`ui-settings-plugins` 拥有“插件”页面与可编辑卡片，`ui-settings-plugin-inventory` 拥有只读清单视图，`ui-settings-plugin-marketplace` 拥有目录/安装/卸载视图，各条 Host／RPC 路径留在各自的拥有包中。新的“插件”视图只需注册一个 `settings.plugins.tab` 贡献即可加入。插件市场决策记录在 [Web 插件市场](../../feature/2026-08-14-web-plugin-marketplace.md)。
