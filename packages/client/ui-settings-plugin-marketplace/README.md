...
The cover lists official profile bundles from [deepseek-ai/deepseek-harness](https://github.com/deepseek-ai/deepseek-harness) as browse-only rows and GitHub repositories tagged `dsh-plugin` as installable `github:owner/repo` specs. Cards show the GitHub owner avatar, description, stars, and language. Open Graph images render in expanded details when the row names a repository. Official rows that cannot be installed carry a browse-only label. Install and remove call `pluginMarketplace.add` / `pluginMarketplace.uninstall`; the Host runs `dsh plugin --profile <name>`. While a mutation is in flight the cover keeps the catalog visible, shows elapsed time, and states that pnpm output is not streamed. A missing Host `pnpm` is a local diagnostic; other mutation failures show the Host message. Notices are dismissible. Copy is Chinese-first. Loading, empty, no-match, source-failure, and generic failure states stay local to the mounted cover. Both registrations use `ctx.slots.inject()`, so they follow late slot declaration, redeclaration, locale changes, and teardown.
