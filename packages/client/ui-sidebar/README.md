...
Sidebar shell plugin: the wordmark, New Session action, layout-owned collapse control, compact bottom nav plus session drawer, layout-switch control, scroll-aware region seat, and bottom-pinned Settings seat. [ui-workspace](../ui-workspace/README.md) owns the Workspace and Session browser rendered into `sidebar.workspaces`; this package neither derives its rows nor owns its view preferences. Collapse into the layout-owned 56px rail remains presentation-local. When the layout owner share sets `presentation` to `drawer`, the shell paints a bottom nav (sessions, New Session, `sidebar.footer.action`, Settings, layout switch) and an overlay session list that Escape, the mask, and a Session change close. Contract: the [slot system standard](../../../.agents/notes/implemented/architecture/2026-07-22-slot-type-chain-implementation.md).
...
`SidebarRootComponentProps` composes the layout owner share, the global `useSessions` and `useWorkspaces` hooks, the declared `sidebar.workspaces` and `sidebar.settings` child slots, and injected `startSession`, sidebar-toggle, and `setShellPreference` callbacks. There is no plugin store.
...
The foot is the `sidebar.settings` seat plus the layout switch: the sidebar renders only the bottom-pinned layout slot and shares its column state (`wide`); ui-settings registers the trigger row and settings panel there. The layout switch stores `ctx.layout.setShellPreference` with the owner share's `nextPreference`.
