...
侧栏 **插件市场** 入口不在 npm CLI 里。在能放下工作区的机器上跑完 `pnpm run build:lib` 后，把已构建包目录（`dsh-host-plugin-marketplace`、`dsh-client-ui-settings-plugin-marketplace`、`dsh-api-remotes`、`dsh-client-connection`、`dsh-client-ui-layout`、`dsh-client-ui-sidebar`、`dsh-client-ui-settings-models`、`dsh-client-ui-conversation`）拷到虚拟机，再执行 `sudo bash deploy/marketplace/install.sh <该目录>`。必须叠加 layout 与 sidebar，`center.cover` 才存在，触发器才会叠在设置上方，compact chrome 才能绘制。叠加 models 与 conversation 会去掉默认内测声明，并把输入框抬到 compact 底栏之上。脚本还会把两个新包装进 `$DSH_HOME/profiles/web/node_modules`，Loader 才能导入它们。再次运行 `install.sh` 会重装 npm 包并丢掉这次叠加；需要再跑一遍市场脚本。细节见[市场包叠加](../.agents/notes/implemented/process/2026-08-14-overlay-marketplace-packages.md)。
