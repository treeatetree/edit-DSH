...
Feature ownership remains explicit: `ui-settings-plugins` owns the Plugins page and editable cards, `ui-settings-plugin-inventory` owns the read-only inventory view, and each Host/RPC path stays in its owning package. A new Plugins view can join by registering one `settings.plugins.tab` contribution. The marketplace lives on a sidebar primary entry and conversation-column cover; that decision is recorded in [plugin marketplace primary entry](../feature/2026-08-14-plugin-marketplace-primary-entry.md).
