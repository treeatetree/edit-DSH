...
用浏览器打开 `http://118.145.156.15/DSH/`。在 **设置 → 模型** 粘贴 DeepSeek API key；叠加层不要求环境里有 `DEEPSEEK_API_KEY`。工作区选 `/opt/dsh/workspace`。`http://118.145.156.15/` 仍是流水首页；`/api` 仍是算账；`/finance` 与 `/hermes` 仍是那些应用。官方客户端仍请求站点根上的 `/api`、`/assets`、`/plugins`。`/DSH/` 的 HTML 会改写 `src="/assets/`、`href="/assets/`（样式表与 modulepreload）、插件 boot URL，以及站点根上的 manifest 与 favicon，并补丁 `fetch` / `WebSocket`。`/DSH/assets/` 下的 CSS 会改写 `url(/assets/`。同时注入 `crypto.randomUUID` polyfill，已发布 CLI 才能在非安全 HTTP 下运行。此 Host 上站点根 `/assets` 是流水 SPA 回退，所以未改写的样式表是 `200 text/html`，而不是 404。nginx 在 `sub_filter` 之后对 `/DSH/assets/` 与 `/DSH/plugins/` 做 gzip；带哈希的资源 7 天后过期。

设置里的 **插件市场** 标签页不在 npm CLI 里。在能放下工作区的机器上跑完 `pnpm run build:lib` 后，把四个已构建包目录（`dsh-host-plugin-marketplace`、`dsh-client-ui-settings-plugin-marketplace`、`dsh-api-remotes`、`dsh-client-connection`）拷到虚拟机，再执行 `sudo bash deploy/marketplace/install.sh <该目录>`。脚本还会把两个新包装进 `$DSH_HOME/profiles/web/node_modules`，Loader 才能导入它们。再次运行 `install.sh` 会重装 npm 包并丢掉这次叠加；需要再跑一遍市场脚本。细节见[市场包叠加](../.agents/notes/implemented/process/2026-08-14-overlay-marketplace-packages.md)。
