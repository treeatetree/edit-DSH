...
The `/client` exports are the plugin body (`apply`/`inject`), `LayoutController`, and the owner-share interfaces (`SidebarOwnerProps`, `ConvOwnerProps`, `CenterCoverOwnerProps`, `DetailsOwnerProps`). AppFrame, the panel store, and the concession solver remain package-internal.
