...
`/client` 导出表层包含插件主体（`apply`／`inject`）、`LayoutController` 和 owner-share 接口（`SidebarOwnerProps`、`ConvOwnerProps`、`CenterCoverOwnerProps`、`DetailsOwnerProps`）。AppFrame、面板 store 与让步求解器仍属于包内部。
